        ...
        if let Some(ref product) = self.state.product {
            html! {
                <div class="product__detail_container">
                    <img class="product__detail_image" src={&product.image}/>
                    <div class="product__card_name">{&product.name}</div>
                    <div style="margin: 10px 0; line-height: 24px;">{&product.description}</div>
                    <div class="product__card_price">{"$"}{&product.price}</div>
                    <AtcButton product=product.clone() on_add_to_cart=self.props.on_add_to_cart.clone() />
                </div>
            }
        } else if !self.state.get_product_loaded {
            html! {
                <div class="loading_spinner__container">
                    <div class="loading_spinner"></div>
                    <div class="loading_spinner__text">{"Loading ..."}</div>
                </div>
            }
        } else {
            html! {
